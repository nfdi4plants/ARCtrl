...
let termFromStringCells (tsrCol : int option) (tanCol : int option ) (cellValues : list<string>) : CompositeCell= 
...
let unitizedFromStringCells (unitCol : int) (tsrCol : int option ) (tanCol : int option) (cellValues : list<string>) : CompositeCell =
    let unit = cellValues.[unitCol]
    let tan = Option.map (fun i -> cellValues.[i]) tanCol
    let tsr = Option.map (fun i -> cellValues.[i]) tsrCol
    CompositeCell.createUnitizedFromString(cellValues.[0],unit,?tsr = tsr, ?tan = tan)

let freeTextFromStringCells (cellValues : list<string>) : CompositeCell =
    CompositeCell.createFreeText cellValues.[0]

let dataFromStringCells (format : int option) (selectorFormat : int option) (cellValues : list<string>) : CompositeCell =
...
let toStringCells isTerm hasUnit (cell : CompositeCell) : list<string> =
    match cell with
