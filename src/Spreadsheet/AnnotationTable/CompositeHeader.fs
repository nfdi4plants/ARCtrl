...
module ActivePattern = 
    ...
    let (|Term|_|) (categoryParser : string -> string option) (f : OntologyAnnotation -> CompositeHeader) (cellValues : string list) : (CompositeHeader*(string list -> CompositeCell)) option =
        let (|AC|_|) s =
            categoryParser s
    ...
    let (|Output|_|) (cellValues : string list) =
        match cellValues with
    ...
    let (|ProtocolHeader|_|) (cellValues : string list) =
        match cellValues with
    ...
    let (|FreeText|_|) (cellValues : string list) =
        match cellValues with
        | [text] -> 
...
let fromStringCells (cellValues : list<string>) : CompositeHeader*(string list -> CompositeCell) =
    match cellValues with
    | Parameter p -> p
    | Factor f -> f
    | Characteristic c -> c
...
let toStringCells (hasUnit : bool) (header : CompositeHeader) : list<string> = 
