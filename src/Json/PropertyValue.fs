    ...
    module ROCrate =
        ...
        let encoder (pv : IPropertyValue) = 
            ...
            let value, valueCode = 
                ...
                | Some v -> 
                    match v with
                    | PropertyValue.Name t -> Some (Encode.string t), None
                    | PropertyValue.Int t -> Some (Encode.int t), None
                    | PropertyValue.Float t -> Some (Encode.float t), None
                    | PropertyValue.Ontology oa -> oa.Name |> Option.map Encode.string, oa.TermAccessionNumber |> Option.map Encode.string
                | None -> None, None
            let unit,unitCode = 
            ...
            Decode.object (fun get ->
                ...
                let value = 
                    ...
                    if value.IsNone && code.IsNone then None
                        ...
                        try PropertyValue.fromOptions value None code
                        with
                        | err -> 
