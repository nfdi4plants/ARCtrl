    ...
    static member composeInvestigation (investigation : ArcInvestigation) =
        ...
        let hasParts =
            [
                ...
                yield! (investigation.Workflows |> ResizeArray.map (fun w -> WorkflowConversion.composeWorkflow w))
                yield! (investigation.Runs |> ResizeArray.map (fun r -> RunConversion.composeRun r))
            ]
            |> ResizeArray
            |> Option.fromSeq
        let mentions =
