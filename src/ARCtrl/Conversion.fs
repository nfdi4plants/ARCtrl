...
type InvestigationConversion =
    ...
    static member composeInvestigation (investigation : ArcInvestigation) =
        ...
        let hasParts =
            [
                yield! (investigation.Assays |> ResizeArray.map (fun a -> AssayConversion.composeAssay a))
                yield! (investigation.Studies |> ResizeArray.map (fun s -> StudyConversion.composeStudy s))
                yield! (investigation.Workflows |> ResizeArray.map (fun w -> WorkflowConversion.composeWorkflowFromArcWorkflow w))
                yield! (investigation.Runs |> ResizeArray.map (fun r -> WorkflowConversion.composeRunFromArcRun r))
            ]
            |> ResizeArray
            |> Option.fromSeq
        let mentions =
