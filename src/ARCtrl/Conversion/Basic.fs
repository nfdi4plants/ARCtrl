...
type BaseTypes = 
    ...
    static member decomposePropertyValue (pv : LDNode, ?context : LDContext) : OntologyAnnotation*PropertyValue option*OntologyAnnotation option =
        ...
        let value =           
            ...
            match v,vRef with
            | _, Some vr ->
                Some (PropertyValue.Ontology (OntologyAnnotation.fromTermAnnotation(vr,?name = v)))
            | Some v, None ->
                PropertyValue.Name v |> Some
            | None, None ->
                None
        let unit =
            let u = LDPropertyValue.tryGetUnitTextAsString(pv, ?context = context)
