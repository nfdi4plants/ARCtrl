...
module Update =
    ...
    let inline mergeUpdateLists (updateOptions : UpdateOptions) (mapping : 'T -> 'Key) (list1 : 'T list) (list2 : 'T list) = 
        try
            let map1 = list1 |> List.map (fun v -> mapping v, v) |> Dictionary.ofSeqWithMerge updateOptions.updateRecordType
            let map2 = list2 |> List.map (fun v -> mapping v, v) |> Dictionary.ofSeqWithMerge updateOptions.updateRecordType
            List.append (list1 |> List.map mapping) (list2 |> List.map mapping)
            |> List.distinct
            |> List.map (fun k ->
                match Dictionary.tryFind k map1, Dictionary.tryFind k map2 with
