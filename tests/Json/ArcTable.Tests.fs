...
module Helper =
    ...
    let create_filled() = ArcTable.create(
        ...
        ResizeArray [
            ResizeArray [CompositeCell.FreeText "Input 1"; CompositeCell.FreeText "Input 2" ];
            ResizeArray [CompositeCell.Term (OntologyAnnotation("SCIEX instrument model")); CompositeCell.Term (OntologyAnnotation("SCIEX instrument model"))];
            ResizeArray [CompositeCell.FreeText "Output 1"; CompositeCell.FreeText "Output 2" ]
        ]
    )
...
let private tests_compressedExtended = testList "compressed_extended" [
        testCase "rangeColumnSize" <| fun _ ->
            ...
            testTable.AddColumn (CompositeHeader.FreeText "ABC", ResizeArray [|for i = 0 to 200 do yield CompositeCell.FreeText "TestValue"|])
            let referenceTable = ArcTable.init("Test")
            referenceTable.AddColumn (CompositeHeader.Input IOType.Source, ResizeArray [|for i = 0 to 100 do yield CompositeCell.FreeText "TestValue"|])
        ...
        testCase "rangeColumnCorrectness" <| fun _ ->
            ...
            for i = 0 to 100 do testTable.AddRow (ResizeArray [|CompositeCell.FreeText "TestValue1"|])
            for i = 101 to 200 do testTable.AddRow (ResizeArray [|CompositeCell.FreeText "TestValue2"|])
            testTable.AddRow (ResizeArray [|CompositeCell.FreeText "TestValue3"|])
        ...
        testCase "SeparateOntologyAnnotations" <| fun _ ->
            ...
            table.AddColumn(
                ...
                ResizeArray [|for i in 0 .. 2 do CompositeCell.createTermFromString $"Term"|]
            )
...
let private tests_dataColumns = 
    testList "dataColumns" [
        testCase "basic WriteRead" <| fun _ ->
            let testTable = ArcTable.init("Test") 
            testTable.AddColumn (CompositeHeader.Input IOType.Data, ResizeArray [|CompositeCell.Data (Data(name="MyInputDataFile.csv"))|])
            testTable.AddColumn (CompositeHeader.Output IOType.Data, ResizeArray [|CompositeCell.Data (Data(name="MyData.csv#row=1",format="text/csv",selectorFormat = "MySelector"))|])
...
let private tests_commentColumns = 
    testList "commentColumns" [
        testCase "basic WriteRead" <| fun _ ->
            let testTable = ArcTable.init("Test") 
            testTable.AddColumn (CompositeHeader.Input IOType.Source, ResizeArray [|CompositeCell.FreeText "Source"|])
            testTable.AddColumn (CompositeHeader.Comment "CommentKey", ResizeArray [|CompositeCell.FreeText "CommentValue"|])
            testTable.AddColumn (CompositeHeader.Output IOType.Sample, ResizeArray [|CompositeCell.FreeText "Sample"|])
            let encoded = testTable.ToJsonString()
            let decoded = ArcTable.fromJsonString encoded
