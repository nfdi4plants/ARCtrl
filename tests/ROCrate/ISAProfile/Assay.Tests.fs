...
let tests_profile_object_is_valid = testList "constructed properties" [
    testList "mandatory properties" [
        testCase "Id" <| fun _ -> Expect.LDNodeHasId "assay_mandatory_properties_id" mandatory_properties
        testCase "SchemaType" <| fun _ -> Expect.LDNodeHasType "schema.org/Dataset" mandatory_properties
        testCase "AdditionalType" <| fun _ -> Expect.LDNodeHasAdditionalType "Assay" mandatory_properties
    ...
    testList "all properties" [
        testCase "Id" <| fun _ -> Expect.LDNodeHasId "assay_all_properties_id" all_properties
        testCase "SchemaType" <| fun _ -> Expect.LDNodeHasType "schema.org/Dataset" all_properties
        testCase "AdditionalType" <| fun _ -> Expect.LDNodeHasAdditionalType "Assay" all_properties
        ...
        testCase "measurementMethod" <| fun _ -> Expect.LDNodeHasDynamicProperty "measurementMethod" "measurementMethod" all_properties
        testCase "measurementTechnique" <| fun _ -> Expect.LDNodeHasDynamicProperty "measurementTechnique" "measurementTechnique" all_properties
        testCase "url" <| fun _ -> Expect.LDNodeHasDynamicProperty "url" "url" all_properties
        testCase "variableMeasured" <| fun _ -> Expect.LDNodeHasDynamicProperty "variableMeasured" "variableMeasured" all_properties
    ]
]

let tests_interface_members = testList "interface members" [
    testCase "mandatoryProperties" <| fun _ -> Expect.LDNodeHasExpectedInterfaceMembers [|"schema.org/Dataset"|] "assay_mandatory_properties_id" [|"Assay"|] mandatory_properties
    testCase "allProperties" <| fun _ -> Expect.LDNodeHasExpectedInterfaceMembers [|"schema.org/Dataset"|] "assay_all_properties_id" [|"Assay"|] all_properties
]
