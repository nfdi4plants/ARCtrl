...
let tests_profile_object_is_valid = testList "constructed properties" [
    testList "mandatory properties" [
        testCase "Id" <| fun _ -> Expect.LDNodeHasId "scholarlyarticle_mandatory_properties_id" mandatory_properties
        testCase "SchemaType" <| fun _ -> Expect.LDNodeHasType "schema.org/ScholarlyArticle" mandatory_properties
    ...
    testList "all properties" [
        ...
        testCase "headline" <| fun _ -> Expect.LDNodeHasDynamicProperty "headline" "headline" all_properties
        testCase "identifier" <| fun _ -> Expect.LDNodeHasDynamicProperty "identifier" "identifier" all_properties
        testCase "author" <| fun _ -> Expect.LDNodeHasDynamicProperty "author" "author" all_properties
        testCase "url" <| fun _ -> Expect.LDNodeHasDynamicProperty "url" "url" all_properties
        testCase "creativeWorkStatus" <| fun _ -> Expect.LDNodeHasDynamicProperty "creativeWorkStatus" "creativeWorkStatus" all_properties
...
let tests_interface_members = testList "interface members" [
    testCase "mandatoryProperties" <| fun _ -> Expect.LDNodeHasExpectedInterfaceMembers [|"schema.org/ScholarlyArticle"|] "scholarlyarticle_mandatory_properties_id" [||] mandatory_properties
    testCase "allProperties" <| fun _ -> Expect.LDNodeHasExpectedInterfaceMembers [|"schema.org/ScholarlyArticle"|] "scholarlyarticle_all_properties_id" [|"additionalType"|] all_properties
]
