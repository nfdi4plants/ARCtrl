...
import { AssayMaterials_get_empty, AssayMaterials_create_1CB3546D } from "../JsonTypes/AssayMaterials.js";
...
import { Assay_create_ABF59A4 } from "../JsonTypes/Assay.js";
...
export class ArcAssay {
    constructor(identifier, measurementType, technologyType, technologyPlatform, tables, performers, comments) {
        ...
        const performers_1 = defaultArg(performers, empty());
    ...
    ToAssay() {
        ...
        let assayMaterials;
        const v_2 = AssayMaterials_create_1CB3546D(unwrap(fromValueWithDefault(empty(), getSamples(processSeq))), unwrap(fromValueWithDefault(empty(), getMaterials(processSeq))));
        assayMaterials = fromValueWithDefault(AssayMaterials_get_empty(), v_2);
        return Assay_create_ABF59A4(void 0, unwrap(isMissingIdentifier(this$.Identifier) ? void 0 : Assay_fileNameFromIdentifier(this$.Identifier)), unwrap(this$.MeasurementType), unwrap(this$.TechnologyType), unwrap(this$.TechnologyPlatform), unwrap(fromValueWithDefault(empty(), getData(processSeq))), unwrap(assayMaterials), unwrap(fromValueWithDefault(empty(), getCharacteristics(processSeq))), unwrap(fromValueWithDefault(empty(), getUnits(processSeq))), unwrap(fromValueWithDefault(empty(), processSeq)), unwrap(fromValueWithDefault(empty(), this$.Comments)));
    }
    static fromAssay(a) {
        const tables = map((arg_1) => ArcTables__get_Tables(ArcTables_fromProcesses_Z31821267(arg_1)), a.ProcessSequence);
        let identifer;
...
export function ArcAssay_$ctor_4D629C41(identifier, measurementType, technologyType, technologyPlatform, tables, performers, comments) {
