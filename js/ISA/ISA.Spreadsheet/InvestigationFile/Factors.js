...
import { Factor_create_Z3D2B374F, Factor_make } from "../../ISA/JsonTypes/Factor.js";
...
export function fromString(name, designType, typeTermSourceREF, typeTermAccessionNumber, comments) {
    const factorType = OntologyAnnotation_fromString_Z7D8EB286(designType, typeTermSourceREF, typeTermAccessionNumber);
    return Factor_make(void 0, Option_fromValueWithDefault("", name), Option_fromValueWithDefault(OntologyAnnotation_get_empty(), factorType), Option_fromValueWithDefault(empty(), comments));
}
...
export function toSparseTable(factors) {
    ...
    iterateIndexed((i, f) => {
        const i_1 = (i + 1) | 0;
        const ft = OntologyAnnotation_toString_473B9D79(defaultArg(f.FactorType, OntologyAnnotation_get_empty()), true);
