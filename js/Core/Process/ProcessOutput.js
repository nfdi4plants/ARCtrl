...
export function ProcessOutput_createSample_Z598187B7(name, characteristics, factors, derivesFrom) {
    return new ProcessOutput(0, [Sample_create_62424AD2(undefined, name, unwrap(characteristics), unwrap(factors), unwrap(derivesFrom))]);
}

export function ProcessOutput_createMaterial_4452CB4C(name, characteristics, derivesFrom) {
    return new ProcessOutput(2, [Material_create_Z66909A6D(undefined, name, undefined, unwrap(characteristics), unwrap(derivesFrom))]);
}

export function ProcessOutput_createImageFile_Z721C83C5(name) {
    return new ProcessOutput(1, [Data.create(undefined, name, new DataFile(2, []))]);
}

export function ProcessOutput_createRawData_Z721C83C5(name) {
    return new ProcessOutput(1, [Data.create(undefined, name, new DataFile(0, []))]);
}

export function ProcessOutput_createDerivedData_Z721C83C5(name) {
    return new ProcessOutput(1, [Data.create(undefined, name, new DataFile(1, []))]);
}
