import { reverse, cons, iterate, iterateIndexed, map as map_1, initialize, singleton, length, empty, ofSeq, ofArray } from "../../fable_modules/fable-library-js.4.22.0/List.js";
...
export function fromString(name, protocolType, typeTermAccessionNumber, typeTermSourceREF, description, uri, version, parametersName, parametersTermAccessionNumber, parametersTermSourceREF, componentsName, componentsType, componentsTypeTermAccessionNumber, componentsTypeTermSourceREF, comments) {
    ...
    return Protocol_make(undefined, map(URIModule_fromString, name), Option_fromValueWithDefault(new OntologyAnnotation(), protocolType_1), description, map(URIModule_fromString, uri), version, Option_fromValueWithDefault(empty(), parameters), Option_fromValueWithDefault(empty(), components), Option_fromValueWithDefault(empty(), comments));
}

export function fromSparseTable(matrix) {
    if ((matrix.ColumnCount === 0) && (length(matrix.CommentKeys) !== 0)) {
        return singleton(Protocol_create_Z414665E7(undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, ofSeq(SparseTable_GetEmptyComments_3ECCA699(matrix))));
    }
    else {
...
export function toSparseTable(protocols) {
    const matrix = SparseTable_Create_Z2192E64B(undefined, labels, undefined, length(protocols) + 1);
