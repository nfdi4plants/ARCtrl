...
export function ProtocolTransformation__Transform_4D8AC666(this$: ProtocolTransformation_$union, p: Protocol): Protocol {
    switch (this$.tag) {
...
export function ProtocolTransformation__Equals_4D8AC666(this$: ProtocolTransformation_$union, p: Protocol): boolean {
...
export function ProcessTransformation__Transform_30EF9E7B(this$: ProcessTransformation_$union, p: Process): Process {
    let pro: Protocol;
    switch (this$.tag) {
        case /* AddParameter */ 1: {
        ...
        case /* AddProtocol */ 4: {
            ...
            return new Process(p.ID, p.Name, fold<ProtocolTransformation_$union, Protocol>((pro_2: Protocol, trans: ProtocolTransformation_$union): Protocol => ProtocolTransformation__Transform_4D8AC666(trans, pro_2), defaultArg(p.ExecutesProtocol, Protocol_get_empty()), pts), p.ParameterValues, p.Performer, p.Date, p.PreviousProcess, p.NextProcess, p.Inputs, p.Outputs, p.Comments);
        }
        default:
...
export function ProcessTransformation__Equals_30EF9E7B(this$: ProcessTransformation_$union, p: Process): boolean {
    const matchValue: Option<string> = p.Name;
    let matchResult: int32, n_1: string, n$0027_1: string;
    if (this$.tag === /* AddName */ 0) {
        if (matchValue != null) {
...
export function AssayTransformation__Transform_Z269B5B97(this$: AssayTransformation_$union, a: Assay): Assay {
    switch (this$.tag) {
        case /* AddProcess */ 4: {
            ...
            return new Assay(a.ID, a.FileName, a.MeasurementType, a.TechnologyType, a.TechnologyPlatform, a.DataFiles, a.Materials, a.CharacteristicCategories, a.UnitCategories, exists<Process>((p: Process): boolean => exists<ProcessTransformation_$union>((trans: ProcessTransformation_$union): boolean => ProcessTransformation__Equals_30EF9E7B(trans, p), pts), processes) ? map_1<Process, Process>((p_1: Process): Process => {
                if (exists<ProcessTransformation_$union>((trans_1: ProcessTransformation_$union): boolean => ProcessTransformation__Equals_30EF9E7B(trans_1, p_1), pts)) {
                    return fold<ProcessTransformation_$union, Process>((p_2: Process, trans_2: ProcessTransformation_$union): Process => ProcessTransformation__Transform_30EF9E7B(trans_2, p_2), p_1, pts);
                }
                else {
                    return p_1;
                }
            }, processes) : append(processes, singleton(fold<ProcessTransformation_$union, Process>((p_3: Process, trans_3: ProcessTransformation_$union): Process => ProcessTransformation__Transform_30EF9E7B(trans_3, p_3), Process_get_empty(), pts))), a.Comments);
        }
        case /* AddFileName */ 0:
...
export function AssayTransformation__Equals_Z269B5B97(this$: AssayTransformation_$union, a: Assay): boolean {
...
export function StudyTransformation__Transform_Z27CB2981(this$: StudyTransformation_$union, s: Study): Study {
    switch (this$.tag) {
        case /* AddProcess */ 3: {
            const pts: FSharpList<ProcessTransformation_$union> = this$.fields[0];
            const processes: FSharpList<Process> = defaultArg(s.ProcessSequence, empty<Process>());
            return new Study(s.ID, s.FileName, s.Identifier, s.Title, s.Description, s.SubmissionDate, s.PublicReleaseDate, s.Publications, s.Contacts, s.StudyDesignDescriptors, s.Protocols, s.Materials, exists<Process>((p: Process): boolean => exists<ProcessTransformation_$union>((trans: ProcessTransformation_$union): boolean => ProcessTransformation__Equals_30EF9E7B(trans, p), pts), processes) ? map_1<Process, Process>((p_1: Process): Process => {
                if (exists<ProcessTransformation_$union>((trans_1: ProcessTransformation_$union): boolean => ProcessTransformation__Equals_30EF9E7B(trans_1, p_1), pts)) {
                    return fold<ProcessTransformation_$union, Process>((p_2: Process, trans_2: ProcessTransformation_$union): Process => ProcessTransformation__Transform_30EF9E7B(trans_2, p_2), p_1, pts);
                }
                else {
            ...
            }, processes) : append(processes, singleton(fold<ProcessTransformation_$union, Process>((p_3: Process, trans_3: ProcessTransformation_$union): Process => ProcessTransformation__Transform_30EF9E7B(trans_3, p_3), Process_get_empty(), pts))), s.Assays, s.Factors, s.CharacteristicCategories, s.UnitCategories, s.Comments);
        }
        case /* AddAssay */ 4: {
            ...
            return new Study(s.ID, s.FileName, s.Identifier, s.Title, s.Description, s.SubmissionDate, s.PublicReleaseDate, s.Publications, s.Contacts, s.StudyDesignDescriptors, s.Protocols, s.Materials, s.ProcessSequence, exists<Assay>((a: Assay): boolean => exists<AssayTransformation_$union>((trans_4: AssayTransformation_$union): boolean => AssayTransformation__Equals_Z269B5B97(trans_4, a), ats), assays) ? map_1<Assay, Assay>((a_1: Assay): Assay => {
                if (exists<AssayTransformation_$union>((trans_5: AssayTransformation_$union): boolean => AssayTransformation__Equals_Z269B5B97(trans_5, a_1), ats)) {
                    return fold<AssayTransformation_$union, Assay>((a_2: Assay, trans_6: AssayTransformation_$union): Assay => AssayTransformation__Transform_Z269B5B97(trans_6, a_2), a_1, ats);
                }
                else {
                    return a_1;
                }
            }, assays) : append(assays, singleton(fold<AssayTransformation_$union, Assay>((a_3: Assay, trans_7: AssayTransformation_$union): Assay => AssayTransformation__Transform_Z269B5B97(trans_7, a_3), Assay_get_empty(), ats))), s.Factors, s.CharacteristicCategories, s.UnitCategories, s.Comments);
        }
        default:
            return toFail(printf("Builder failed: Case %O Not implemented"))(this$);
    }
